...
#message("git output: " $$GIT_VERSION)
...
!contains(GIT_VERSION, \d+"."\d+"."\d+.*) {
    ...
    message("git doesnt contains tag versioning.")
    isEmpty("$${GIT_VERSION}") {
        GIT_VERSION = $$VERSION
...
DEFINES += APP_FULL_VERSION=\\\"$$GIT_VERSION\\\"
DEFINES += APP_SMALL_VERSION=\\\"$$section(GIT_VERSION, ., 0, 3)\\\"
